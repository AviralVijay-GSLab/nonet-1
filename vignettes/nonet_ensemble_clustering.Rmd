...
```{r, warning = FALSE}
plot_first <- nonet_plot(Prediction_dataframe$pred_nonet, y = NULL, Prediction_dataframe, plot_type = "hist")
```
...
```{r, warning = FALSE}
plot_second <- nonet_plot(Prediction_dataframe$pred_clust_first,  y = NULL, Prediction_dataframe, plot_type = "hist")
plot_second
```
```{r, warning = FALSE}
plot_third <- nonet_plot(Prediction_dataframe$pred_clust_second,  y = NULL, Prediction_dataframe, plot_type = "hist")
plot_third
```
...
```{r}
#confusionMatrix(prediction_nonet_class, trainSet[,outcomeName])
```
```{r}
#confusionMatrix(predict_cluster_first_class,trainSet[,outcomeName])

```

```{r}
#confusionMatrix(predict_cluster_Second_class,trainSet[,outcomeName])
```
